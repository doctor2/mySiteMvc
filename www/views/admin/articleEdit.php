...
<form id="editRecord" name="editRecord" method="post" action="" >
	<p>
		...
		<input type="text" name="title" id="title" size="100" value = "<?=$this->record['title']?>" />
	</p>
	<p>
		<label for="date">Дата создания заметки</label>
		<input type="date" name="date" id="date" value="<?=$this->record['date']?>">
	</p>
	<label for="content"> Текст заметки </label>
	<textarea name="content" id="content" cols="140" rows="19"><?=$this->record['content']?></textarea>
